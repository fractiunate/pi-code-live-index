...
from pi_code_index import backend, context_tools, indexer
...
def test_internal_context_tool_payloads(tmp_path: Path, monkeypatch) -> None:
    repo = make_repo(tmp_path)
    indexer.refresh(repo)

    repo_map = context_tools.repo_map(repo, "src", depth=9, backend="cocoindex")
    assert repo_map["ok"] is True
    assert repo_map["depth"] == 5
    assert repo_map["capabilities"]["repo_map"] is True
    ...
    tests = context_tools.find_tests(repo, ["src/pkg/config.py"], top_k=999, backend="cocoindex")
    assert tests["top_k"] == 100
    assert tests["results"][0]["test_file"] == "tests/test_config.py"
    assert tests["results"][0]["metadata"]["source"] in {"heuristic-v1", "test-link-heuristic-v1"}

    similar = context_tools.find_similar_code(repo, query="load config", top_k=5, backend="cocoindex")
    assert similar["results"]

    review = context_tools.review_context(repo, ["src/pkg/cli.py"], backend="cocoindex")
...
def test_find_similar_code_lexical_symbols_scope_warns_and_returns_chunks(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "symbols unavailable" in payload["warning"]
...
def test_find_tests_token_overlap_ranks_renamed_module_test(tmp_path: Path, monkeypatch) -> None:
    ...
    indexer.refresh(repo)
    payload = context_tools.find_tests(repo, ["src/retry_policy.py"], top_k=10)
...
def test_find_similar_code_components_and_evidence_present(tmp_path: Path, monkeypatch) -> None:
    repo = make_retry_repo(tmp_path)
    indexer.refresh(repo)
    result = context_tools.find_similar_code(repo, query="retry backoff helper", mode="hybrid", top_k=3)["results"][0]
...
"""
    ...
    repo_map = context_tools.repo_map(repo, "src/retry.py", depth=2, refresh_first=True, backend="cocoindex")
    ...
    review = context_tools.review_context(repo, ["src/retry.py"], top_k=30, refresh_first=True, backend="cocoindex")
